...
/**
 * Unit-level coverage of listAccountFindings' query-building (account_id
 ...
 * `variance_finding.account_id = $1` predicate (86e31a9ch/#216 precedent)
...
describe('listAccountFindings (unit, mocked client)', () => {
  it('maps a returned row from snake_case columns to the AccountFindingRow shape', async () => {
    const { client } = mockClient([mockRow()]);
    const rows = await listAccountFindings(client, CLIENT_ID, {});
    expect(rows).toEqual([
      {
        id: 'f1',
        auditRunId: 'r1',
  ...
  it('always binds clientId as the first param, ahead of any other filter', async () => {
    ...
    await listAccountFindings(client, CLIENT_ID, { carrier: 'ACME', status: 'open', minAmount: '100.00' });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(sql).toMatch(/WHERE variance_finding\.account_id = \$1 AND carrier\.name = \$2 AND variance_finding\.status = \$3::variance_status AND variance_finding\.variance_amount >= \$4/);
    expect(params).toEqual([CLIENT_ID, 'ACME', 'open', '100.00', 50, 0]);
  });

  it('builds only the account_id predicate and uses default LIMIT/OFFSET when no other filters are given', async () => {
    const { client, query } = mockClient([]);
    await listAccountFindings(client, CLIENT_ID, {});
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    const afterLastLateral = sql.split('criterion_version ON true')[1]!;
    expect(afterLastLateral).toMatch(/WHERE variance_finding\.account_id = \$1\s*$/m);
    expect(params).toEqual([CLIENT_ID, 50, 0]);
  });

  it('adds a carrier condition and binds it positionally after clientId', async () => {
    const { client, query } = mockClient([]);
    await listAccountFindings(client, CLIENT_ID, { carrier: 'ACME' });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
  ...
  it('honors explicit limit and offset', async () => {
    const { client, query } = mockClient([]);
    await listAccountFindings(client, CLIENT_ID, { limit: 10, offset: 20 });
  ...
  it('has no clientIds field in ListAccountFindingsOptions -- the client scope is bound structurally by the clientId param, never a caller-supplied filter', async () => {
    const { client, query } = mockClient([]);
    // @ts-expect-error -- clientIds is deliberately not part of this options shape (unlike list-findings.ts's ListFindingsOptions).
    await listAccountFindings(client, CLIENT_ID, { clientIds: ['other-client'] });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
  ...
  describe('sort', () => {
    ...
    it('sorts by variance_amount, ASC, with NULLS LAST', async () => {
      const { client, query } = mockClient([]);
      await listAccountFindings(client, CLIENT_ID, { sort: 'variance', sortDir: 'asc' });
      const [sql] = query.mock.calls[0] as [string, unknown[]];
    ...
    it('sorts by created_at when sort: "age" is requested, with no NULLS LAST', async () => {
      const { client, query } = mockClient([]);
      await listAccountFindings(client, CLIENT_ID, { sort: 'age', sortDir: 'asc' });
      const [sql] = query.mock.calls[0] as [string, unknown[]];
    ...
    it('ORDER BY always comes before the outer LIMIT/OFFSET, regardless of sort', async () => {
      const { client, query } = mockClient([]);
      await listAccountFindings(client, CLIENT_ID, { sort: 'variance', sortDir: 'asc', limit: 10, offset: 5 });
      const [sql] = query.mock.calls[0] as [string, unknown[]];
