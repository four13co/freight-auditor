...
describe('updateFindingStatus (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const a = await owner.query(`INSERT INTO account (name, slug) VALUES ('UFS-A', $1) RETURNING id`, [`${tag}-a`]);
      clientAId = a.rows[0].id;
      const b = await owner.query(`INSERT INTO account (name, slug) VALUES ('UFS-B', $1) RETURNING id`, [`${tag}-b`]);
  ...
  async function seedFinding(
    ...
    const vf = await client.query(
      `INSERT INTO variance_finding
       ...
       SELECT $1, $2, $3, c.id, rv.id, 'OVERCHARGE', $4, 'USD', $5, '{}'::jsonb
