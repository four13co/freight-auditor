...
/**
 * Rebuild of a PR closed for a test-teardown/fixture FK bug: the original
 * beforeAll never inserted an app_user row and instead used a hardcoded
 * literal UUID as the approving actor. approveDispute's writeAuditEvent
 * call inserts actor_user_id referencing that literal, which violates
 * audit_event's FK to app_user(id) since no such row exists -- a genuine
 * 23503 in CI, not a flake. Fixed by seeding a real app_user row (same
 * convention as claim-endpoint.db.test.ts) and using its returned id.
 *
