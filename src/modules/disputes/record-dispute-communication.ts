 ...
 * INSERT ... ON CONFLICT (account_id, dedupe_key) DO NOTHING, then a plain
 ...
 *
 * Looks up the dispute's own account_id (RLS-scoped, `WHERE id = $1` with no
 ...
 * updateFindingStatus's own `UPDATE ... WHERE id = $1 RETURNING account_id`
...
export async function recordDisputeCommunication(
  ...
  const dispute = await client.query<{ account_id: string }>(
    `SELECT account_id FROM dispute WHERE id = $1`,
    [input.disputeId],
  );
  const disputeRow = dispute.rows[0];
  if (!disputeRow) throw new RecordDisputeCommunicationError('DISPUTE_NOT_FOUND');
  const clientId = disputeRow.account_id;

  const inserted = await client.query<{ id: string }>(
    `INSERT INTO dispute_comm (account_id, dispute_id, direction, body, dedupe_key)
     VALUES ($1, $2, $3, $4, $5)
  ...
  const existing = await client.query<{ id: string }>(
    `SELECT id FROM dispute_comm WHERE account_id = $1 AND dedupe_key = $2`,
