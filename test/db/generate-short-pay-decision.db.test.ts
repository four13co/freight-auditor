 ...
 *
 * GOLDEN_210's own findings are all STD-prefixed/INTEGRITY_ONLY (no
 * CONTRACT.RATE_VARIANCE/OVERCHARGE finding exists in that fixture), so the
 * one accepted-overcharge finding this test needs is seeded directly via
 * the resolver-join pattern (criterion/rule/rule_version resolved by
 * criterion_key, not hand-picked ids) -- required because migration 0025's
 * variance_finding_refs_required check constraint makes criterion_id/
 * rule_version_id NOT NULL; a hand-rolled INSERT omitting them violates
 * that constraint (this item's prior review-closure defect). Teardown also
 * deletes finding_status_event/scorecard before audit_run, which the prior
 * attempt's teardown was missing.
 */
