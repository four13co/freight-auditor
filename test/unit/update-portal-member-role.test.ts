...
function mockClient(rows: Array<{ id: string; account_id: string; from_role: string }>) {
  const query = vi.fn()
...
describe('updatePortalMemberRole (unit, mocked client)', () => {
  it('returns found: true and passes membershipId/clientId/newRole/portal-roles as positional params', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, from_role: 'account_viewer' }]);
    const result = await updatePortalMemberRole(client, clientId, membershipId, 'account_admin', '10000000-0000-4000-8000-000000000099');
    expect(result).toEqual({ found: true });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(params).toEqual([membershipId, clientId, 'account_admin', ['account_viewer', 'account_admin']]);
  ...
  it('returns found: false when the UPDATE affects zero rows (missing, cross-tenant, or a non-portal current role)', async () => {
    ...
    const result = await updatePortalMemberRole(client, clientId, 'missing-id', 'account_admin', '10000000-0000-4000-8000-000000000099');
    expect(result).toEqual({ found: false });
  });

  it('never issues the audit-event write when the UPDATE found no row', async () => {
    const { client, query } = mockClient([]);
    await updatePortalMemberRole(client, clientId, 'missing-id', 'account_admin', '10000000-0000-4000-8000-000000000099');
    expect(query).toHaveBeenCalledTimes(1);
  });

  it('casts newRole to membership_role in the query', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, from_role: 'account_admin' }]);
    await updatePortalMemberRole(client, clientId, membershipId, 'account_viewer', '10000000-0000-4000-8000-000000000099');
  ...
  it('writes an audit event carrying the from/to role transition after a successful update', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, from_role: 'account_viewer' }]);
    await updatePortalMemberRole(client, clientId, membershipId, 'account_admin', '10000000-0000-4000-8000-000000000099');
    ...
    expect(auditParams[4]).toBe('membership.role_changed_to_account_admin');
    expect(auditParams[5]).toBe('client');
    expect(auditParams[6]).toBe('10000000-0000-4000-8000-000000000099');
    expect(auditParams[9]).toEqual({ fromRole: 'account_viewer', toRole: 'account_admin' });
  });
});
