...
describe('SFTP connector checkpoints (DB)', () => {
  ...
  beforeAll(async () => {
    pool = getPool();
    const a = await pool.query(`INSERT INTO account (name, slug) VALUES ('SFTP A', $1) RETURNING id`, [`${tag}-a`]);
    const b = await pool.query(`INSERT INTO account (name, slug) VALUES ('SFTP B', $1) RETURNING id`, [`${tag}-b`]);
    clientA = a.rows[0].id;
    clientB = b.rows[0].id;
    const connection = await pool.query(
      `INSERT INTO sftp_connection
        (account_id, name, host, username, remote_path, private_key_secret_ref, host_key_sha256)
       VALUES ($1, 'primary', 'sftp.example.test', 'freight', '/inbound', 'SFTP_KEY_A', $2) RETURNING id`,
      [clientA, 'b'.repeat(64)],
    );
    connectionA = connection.rows[0].id;
    await pool.query(
      `INSERT INTO sftp_intake (account_id, connection_id, remote_path, remote_fingerprint, status)
  ...
  afterAll(async () => {
    await pool.query(`DELETE FROM sftp_intake WHERE account_id IN ($1, $2)`, [clientA, clientB]);
    await pool.query(`DELETE FROM sftp_connection WHERE account_id IN ($1, $2)`, [clientA, clientB]);
