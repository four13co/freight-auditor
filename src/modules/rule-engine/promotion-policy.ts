...
export async function upsertPromotionPolicy(client: pg.PoolClient, untrusted: unknown): Promise<PromotionPolicy> {
  ...
  const v = parsed.data;
  const row = (await client.query<{ account_id: string; rule_type: PromotionPolicy['ruleType']; n1_confirm: number; n2_confirm: number; max_reversals: number }>(
    `INSERT INTO promotion_policy (account_id, rule_type, n1_confirm, n2_confirm, max_reversals)
     VALUES ($1,$2,$3,$4,$5) ON CONFLICT (account_id, rule_type) DO UPDATE SET
       n1_confirm=EXCLUDED.n1_confirm, n2_confirm=EXCLUDED.n2_confirm, max_reversals=EXCLUDED.max_reversals
     RETURNING account_id, rule_type, n1_confirm, n2_confirm, max_reversals`,
    [v.clientId, v.ruleType, v.n1Confirm, v.n2Confirm, v.maxReversals])).rows[0]!;
  return { clientId: row.account_id, ruleType: row.rule_type, n1Confirm: row.n1_confirm, n2Confirm: row.n2_confirm, maxReversals: row.max_reversals };
}
...
export async function resolvePromotionPolicy(client: pg.PoolClient, clientId: string, ruleType: z.infer<typeof RuleType>): Promise<PromotionPolicy> {
  const row = (await client.query<{ account_id: string | null; rule_type: PromotionPolicy['ruleType']; n1_confirm: number; n2_confirm: number; max_reversals: number }>(
    `SELECT account_id, rule_type, n1_confirm, n2_confirm, max_reversals FROM promotion_policy
     WHERE (account_id=$1 OR account_id IS NULL) AND (rule_type=$2 OR rule_type IS NULL)
     ORDER BY (account_id IS NOT NULL) DESC, (rule_type IS NOT NULL) DESC LIMIT 1`, [clientId, ruleType])).rows[0];
