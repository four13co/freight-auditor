...
export async function ratifyContractRuleProposal(client:pg.PoolClient,untrusted:z.input<typeof schema>):Promise<{ratificationId:string;activeRuleVersionId:string;created:boolean}>{
 const input=schema.parse(untrusted); const row=(await client.query<Row>(`SELECT a.id acceptance_id,a.proposal_id,a.backtest_id,a.shadow_rule_version_id shadow_id,
  ...
  JOIN contract_rule_proposal_backtest b ON b.id=a.backtest_id AND b.account_id=a.account_id AND b.proposal_id=a.proposal_id
  WHERE a.account_id=$1 AND a.id=$2 AND rv.lifecycle_state='SHADOW' AND rv.hardness='AI_DOCS' AND b.passed=true
 ...
 const inserted=await client.query<{id:string}>(`INSERT INTO contract_rule_proposal_ratification(account_id,acceptance_id,proposal_id,backtest_id,shadow_rule_version_id,active_rule_version_id,ratified_by,rationale)
  VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(account_id,acceptance_id) DO NOTHING RETURNING id`,[input.clientId,row.acceptance_id,row.proposal_id,row.backtest_id,row.shadow_id,activeId,input.actorUserId,input.rationale]);
 let ratificationId=inserted.rows[0]?.id;
 if(!ratificationId) ratificationId=(await client.query<{id:string}>(`SELECT id FROM contract_rule_proposal_ratification WHERE account_id=$1 AND acceptance_id=$2 AND active_rule_version_id=$3 AND ratified_by=$4 AND rationale=$5`,[input.clientId,row.acceptance_id,activeId,input.actorUserId,input.rationale])).rows[0]?.id;
