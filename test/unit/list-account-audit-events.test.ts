...
describe('listAccountAuditEvents (unit, mocked client)', () => {
  it('maps rows from snake_case to camelCase', async () => {
    ...
    const result = await listAccountAuditEvents(client, CLIENT_ID);
    expect(result).toEqual([
      { id: 'e1', entity: 'dispute', entityId: 'd1', event: 'created', actorKind: 'analyst', recordedAt: new Date('2026-01-01T00:00:00Z') },
    ]);
  });

  it('always includes an explicit account_id = $1 predicate, as $1', async () => {
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID);
  ...
  it('binds an explicit limit and offset, in that order, as the trailing two params', async () => {
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID, { limit: 10, offset: 20 });
  ...
  it('adds an entity filter only when provided', async () => {
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID, { entity: 'dispute' });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
  ...
  it('adds an event filter only when provided', async () => {
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID, { event: 'created' });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
  ...
  it('adds from/to date-range filters only when provided, in order', async () => {
    ...
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID, { from, to });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
  ...
  it('combines all filters with AND, params in call order', async () => {
    const from = new Date('2026-01-01T00:00:00Z');
    const { client, query } = mockClient();
    await listAccountAuditEvents(client, CLIENT_ID, { entity: 'dispute', event: 'created', from, limit: 5, offset: 0 });
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(sql).toMatch(/WHERE account_id = \$1 AND entity = \$2 AND event = \$3 AND recorded_at >= \$4/);
