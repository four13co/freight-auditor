...
describe.skipIf(!DATABASE_URL)('reclaim stale workflow commands (database)', () => {
  ...
  beforeAll(async () => {
    await getPool().query(
      `INSERT INTO account (id, name, slug) VALUES ($1, 'Workflow Recovery Co', $2)`,
  ...
  async function seedClaimedCommand(opts: { attempts: number; claimedMinutesAgo: number }): Promise<string> {
    const { rows } = await getPool().query<{ id: string }>(
      `INSERT INTO workflow_command (account_id, workflow_instance_id, command_type, payload, run_after, status, attempts, claimed_at)
