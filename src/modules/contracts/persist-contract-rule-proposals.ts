...
export async function persistContractRuleProposals(client: pg.PoolClient, untrusted: z.input<typeof inputSchema>): Promise<{
  ...
  const verified = (await client.query<{ source_document_id: string; extraction_response_hash: string; verification_hash: string; sha256: string;
    ...
    FROM verified_contract_version v JOIN source_document s ON s.id=v.source_document_id AND s.account_id=v.account_id
    WHERE v.id=$1 AND v.account_id=$2`, [input.verifiedContractVersionId, input.clientId])).rows[0];
  ...
  const extractionExists = (await client.query(`SELECT 1 FROM audit_event WHERE account_id=$1 AND entity='contract_extraction'
  ...
  const clauseRows = references.length ? (await client.query<{ id: string; clause_ref: string }>(`SELECT id,clause_ref FROM contract_clause
    WHERE account_id=$1 AND contract_version_id=$2 AND clause_ref=ANY($3::text[]) ORDER BY clause_ref`,
  ...
  for (const criterion of input.result.output.criteria) {
    ...
    const inserted = await client.query<{ id: string }>(`INSERT INTO contract_rule_proposal
      (account_id,verified_contract_version_id,criterion_key,kind,rule_type,description,ast,ast_hash,expected_inputs,
    ...
    if (!id) id = (await client.query<{ id: string }>(`SELECT id FROM contract_rule_proposal WHERE account_id=$1 AND proposal_hash=$2
    ...
    for (const reference of [...criterion.clauseReferences].sort()) {
      const relation = await client.query(`INSERT INTO contract_rule_proposal_clause(account_id,proposal_id,contract_clause_id,citations)
        VALUES($1,$2,$3,$4::jsonb) ON CONFLICT(account_id,proposal_id,contract_clause_id) DO NOTHING`,
      [input.clientId, id, clauses.get(reference), JSON.stringify(criterion.citations)]);
      if (relation.rowCount !== 1) {
        const exact = (await client.query(`SELECT 1 FROM contract_rule_proposal_clause WHERE account_id=$1 AND proposal_id=$2
