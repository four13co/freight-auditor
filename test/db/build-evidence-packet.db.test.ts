...
describe('buildEvidencePacket (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const c = await owner.query(`INSERT INTO account (name, slug) VALUES ('EVP', $1) RETURNING id`, [tag]);
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await owner.query(`DELETE FROM computation_trace WHERE account_id = $1`, [clientId]);
  ...
  async function seedDisputeWithFinding(
    ...
    if (opts.withTrace) {
      await client.query(
        `INSERT INTO computation_trace (account_id, audit_run_id, step_order, step, pinned_inputs)
    ...
    const dispute = await client.query<{ id: string }>(
      `INSERT INTO dispute (account_id, carrier_id, status, amount_claimed, currency) VALUES ($1, $2, 'draft', '100.0000', 'USD') RETURNING id`,
      [clientId, carrierId],
    );
    const disputeId = dispute.rows[0]!.id;

    const line = await client.query<{ id: string }>(
      `INSERT INTO dispute_line (account_id, dispute_id, variance_finding_id, amount, currency) VALUES ($1, $2, $3, '100.0000', 'USD') RETURNING id`,
  ...
  it('fails closed for a dispute belonging to a different tenant', async () => {
    ...
    try {
      const c = await otherClient.query(`INSERT INTO account (name, slug) VALUES ('EVP-OTHER', $1) RETURNING id`, [`${tag}-other`]);
    ...
    const owner = await pool.connect();
    try {
      await owner.query(`DELETE FROM account WHERE id = $1`, [otherClientId]);
    } finally {
      owner.release();
    }
  });
