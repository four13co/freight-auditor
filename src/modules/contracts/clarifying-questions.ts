...
const questionSchema = abstentionSchema.omit({ clarificationQuestion: true }).extend({
...
const persistenceInputSchema = z.object({
  ...
  questions: z.array(questionSchema).max(30_000),
}).strict();

export type GeneratedClarifyingQuestion = z.infer<typeof questionSchema>;
...
export function generateClarifyingQuestions(
  ...
  for (const abstention of abstentions) {
    const question = { path: abstention.path, status: abstention.status, reason: abstention.reason,
      ...
      questionHash: hashQuestion(version, abstention.path, abstention.status, abstention.reason, abstention.clarificationQuestion) };
...
export async function persistClarifyingQuestions(
  ...
  for (const question of input.questions) {
    if (question.questionHash !== hashQuestion(input.policyVersion, question.path, question.status, question.reason, question.question)) {
...
function hashQuestion(...parts: string[]): string {
