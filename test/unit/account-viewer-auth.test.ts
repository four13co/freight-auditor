...
describe('resolveAccountViewerContext (DEV_AUTH_HEADERS set)', () => {
  ...
  it('returns null when x-user-id is missing, without querying the DB', async () => {
    ...
    const ctx = await resolveAccountViewerContext(mockRequest({ 'x-account-id': 'client-1' }));
    expect(ctx).toBeNull();
    expect(withTenantTx).not.toHaveBeenCalled();
  });
  ...
  it('grants { clientIds: [clientId], internal: false } under an internal-scoped transaction when the role is client_viewer', async () => {
    ...
    const ctx = await resolveAccountViewerContext(mockRequest({ 'x-account-id': 'client-1', 'x-user-id': 'user-1' }));
...
describe('resolveAccountViewerContext (DEV_AUTH_HEADERS unset -- the prod default)', () => {
  ...
  it('propagates (does not swallow) a genuine backend error when a cookie IS present', async () => {
    ...
    await expect(
      resolveAccountViewerContext(mockRequest({ cookie: 'better-auth.session_token=valid', 'x-account-id': 'client-1' })),
