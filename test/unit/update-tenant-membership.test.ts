 ...
 * real Postgres (the explicit account_id boundary, the audit-event write).
 */
...
function mockClient(rows: Array<{ id: string; account_id: string; role: string; is_active: boolean; from_role: string }>) {
  const query = vi.fn()
...
describe('updateTenantMembership (unit, mocked client)', () => {
  it('AC1: updates the role, scopes the query by account_id, and returns found: true', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, role: 'lead', is_active: true, from_role: 'analyst' }]);
    ...
    expect(sql).toMatch(/WHERE id = \$1 AND account_id = \$2/);
  });
  ...
  it('flips is_active without touching role', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, role: 'analyst', is_active: false, from_role: 'analyst' }]);
  ...
  it('updates both role and is_active in one call', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, role: 'lead', is_active: false, from_role: 'analyst' }]);
  ...
  it('does not write an audit event when the submitted role equals the current role', async () => {
    const { client, query } = mockClient([{ id: membershipId, account_id: clientId, role: 'analyst', is_active: true, from_role: 'analyst' }]);
  ...
  it('no-field call is a pure read scoped by account_id, returning the current row', async () => {
    ...
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(sql).toMatch(/SELECT id, role, is_active FROM membership WHERE id = \$1 AND account_id = \$2/);
