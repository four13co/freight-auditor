...
/**
 * Unit-level coverage of listAccountDisputeCommunications, mirroring
 * list-dispute-communications.test.ts's structure exactly -- the added
 * coverage here is the explicit `account_id` predicate (86e31a9ch/#216
...
describe('listAccountDisputeCommunications (unit, mocked client)', () => {
  it('maps rows to camelCase, newest first per the query order', async () => {
    ...
    const result = await listAccountDisputeCommunications(client, CLIENT_ID, DISPUTE_ID);
    ...
    expect(sql).toMatch(/WHERE dispute_id = \$1 AND account_id = \$2/);
