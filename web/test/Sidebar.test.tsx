...
describe('Sidebar', () => {
  ...
  it('86e36xk28 AC3: nav item and saved-view labels no longer carry font-extrabold', () => {
    ...
    // "Invoices", and "Settings" are now real <a>s, not <button>s -- checked
    // via .closest('a') below, same guarantee, updated selector for the new
    // DOM shape. The remaining still-disabled items are unaffected.
    for (const label of ['Discrepancies', 'Audit log', 'Invoices', 'Settings']) {
      expect(screen.getByText(label).closest('a')).not.toHaveClass('font-extrabold');
    }
    for (const label of ['Mine, over $500', 'Estes accessorials']) {
