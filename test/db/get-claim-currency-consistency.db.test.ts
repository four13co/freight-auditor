...
describe('getClaimCurrencyConsistency (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const a = await owner.query(`INSERT INTO account (name, slug) VALUES ('CCC-A', $1) RETURNING id`, [`${tag}-a`]);
      clientAId = a.rows[0].id;
      const b = await owner.query(`INSERT INTO account (name, slug) VALUES ('CCC-B', $1) RETURNING id`, [`${tag}-b`]);
  ...
  async function seedClaim(client: pg.PoolClient, opts: { clientId: string; currency?: string | null }): Promise<string> {
    const { rows } = await client.query<{ id: string }>(
      `INSERT INTO claim (account_id, amount_claimed, currency, status) VALUES ($1, '500.0000', $2, 'open') RETURNING id`,
  ...
  it('reports consistent when all recovery_event currencies match the claim', async () => {
    const result = await withTenantTx({ clientIds: [clientAId] }, async (client) => {
      ...
      await client.query(`INSERT INTO recovery_event (account_id, claim_id, amount_recovered, currency) VALUES ($1, $2, '100.0000', 'USD')`, [clientAId, claimId]);
  ...
  it('reports inconsistent when a recovery_event currency disagrees with the claim', async () => {
    const result = await withTenantTx({ clientIds: [clientAId] }, async (client) => {
      const claimId = await seedClaim(client, { clientId: clientAId });
      await client.query(`INSERT INTO recovery_event (account_id, claim_id, amount_recovered, currency) VALUES ($1, $2, '100.0000', 'CAD')`, [clientAId, claimId]);
      return getClaimCurrencyConsistency(client, clientAId, claimId);
    });
  ...
  it('surfaces a null-currency recovery_event as its own finding', async () => {
    const result = await withTenantTx({ clientIds: [clientAId] }, async (client) => {
      const claimId = await seedClaim(client, { clientId: clientAId });
      await client.query(`INSERT INTO recovery_event (account_id, claim_id, amount_recovered, currency) VALUES ($1, $2, '100.0000', NULL)`, [clientAId, claimId]);
      return getClaimCurrencyConsistency(client, clientAId, claimId);
    });
