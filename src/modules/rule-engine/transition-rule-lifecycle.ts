...
export async function transitionRuleLifecycle(client: pg.PoolClient, input: {
  ruleVersionId: string; to: RuleLifecycle; rationale: string; backtestRunId?: string | null;
  ...
  await client.query(`INSERT INTO promotion_event
      (rule_version_id, from_hardness, to_hardness, from_lifecycle, to_lifecycle, direction, backtest_run_id, rationale)
  ...
  [nextId, current.lifecycle_state, input.to,
    input.to === 'DEPRECATED' || input.to === 'QUARANTINED' ? 'DEMOTE' : 'PROMOTE', input.backtestRunId ?? null, input.rationale]);
