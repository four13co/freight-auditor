...
export async function persistExtractionFieldCorrection(
  ...
  const original = (await client.query<OriginalField>(`SELECT id,source_document_id,field_path,ai_value,confidence,page_ref,bbox,
    ...
    FROM extraction_field WHERE id=$1 AND account_id=$2 AND extraction_response_hash IS NOT NULL AND correction_hash IS NULL`,
  ...
  const inserted = await client.query<{ id: string }>(`INSERT INTO extraction_field
    (account_id,source_document_id,field_path,ai_value,human_value,confidence,page_ref,bbox,model_version,prompt_version,
    ...
    ON CONFLICT (account_id,source_document_id,extraction_response_hash,field_path,correction_hash)
  ...
  if (!id) id = (await client.query<{ id: string }>(`SELECT id FROM extraction_field WHERE account_id=$1
