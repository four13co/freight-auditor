...
export async function acceptContractRuleProposal(client: pg.PoolClient, untrusted: z.input<typeof inputSchema>): Promise<{
  ...
  const shadowVersion = await insertIdempotent(client, {
    ...
    ON CONFLICT(source_contract_rule_proposal_id,lifecycle_state) WHERE source_contract_rule_proposal_id IS NOT NULL DO NOTHING`,
    ...
    fallbackSql: `SELECT id FROM rule_version WHERE source_contract_rule_proposal_id=$9
    AND rule_id=$10 AND lifecycle_state='SHADOW' AND ast_hash=$11 AND source_contract_rule_proposal_backtest_id=$12
    AND provenance IS NOT DISTINCT FROM $13::jsonb`,
    fallbackParams: [proposal.id, rule.rows[0]!.id, proposal.ast_hash, backtest.id, JSON.stringify(provenance)],
  });
  if (!shadowVersion) throw new ProposalAcceptanceError('ACCEPTANCE_CONFLICT');
