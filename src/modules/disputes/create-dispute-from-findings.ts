...
import { detectDuplicateFindingInclusion } from './detect-duplicate-finding-inclusion.js';
import {
...
export { DuplicateFindingInclusionError } from './detect-duplicate-finding-inclusion.js';
 ...
 * empty dispute. Preventing the SAME finding from being included on two
 * DIFFERENT disputes across time (P4.C.2) is enforced by
 * detectDuplicateFindingInclusion below plus a partial unique index on
 * dispute_line (migration 0051).
 */
export async function createDisputeFromFindings(
  ...
  await detectDuplicateFindingInclusion(client, input.clientId, validated.findingIds);
