 ...
 *     table itself is RLS-scoped by account_id.
 *
...
export interface SourceDocumentRef {
  ...
  created: boolean;
  /**
   * true when the returned row's account_id matches the caller's input.clientId.
...
export async function storeSourceDocument(
  ...
  const existing = await client.query<{ id: string }>(
    `SELECT id FROM source_document WHERE account_id IS NOT DISTINCT FROM $1 AND sha256 = $2`,
