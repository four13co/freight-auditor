...
// precedent.
//
// REBUILD after PR #303's failed review: AC3 previously asserted the
// endpoint returns the finding "to its pre-action state," which
// recordHumanOverrideReversal (src/modules/rule-engine/record-human-override-reversal.ts)
// never does -- reversal is purely human_override/rule-quarantine
// bookkeeping, orthogonal to variance_finding.status. The task's own
// Rabbit-holes text is explicit: "don't assert ledger/quarantine internals
// here, just that the reversal write itself completes and the finding's own
// status is unaffected." This rebuild follows that literally: AC3 below
// asserts (a) the human_override row lands with reversal_count=1 (the AC's
// own stated requirement, not a ledger-internals digression) and (b)
// variance_finding.status is read BEFORE and AFTER the reverse call and
// confirmed IDENTICAL -- not asserting any particular value, just that
// /reverse never touches it. Quarantine/promotion-policy threshold behavior
// (P6.C.8/P6.C.9) is untouched, per the task's own No-gos.
//
...
test.beforeAll(async ({ request }) => {
  ...
  // AC3's precondition -- the seeded finding's rule must resolve to a
  // FIRM_RULE-hardness rule, or /reverse returns 409 NOT_A_FIRM_RULE instead
  // of recording anything. Confirmed once here (failing loudly and
  // specifically, rather than letting AC3 fail with a confusing 409) instead
  // of assumed.
  const hardness = await pool.query<{ hardness: string }>(`SELECT hardness FROM rule_version WHERE id = $1`, [ruleVersionId]);
  if (hardness.rows[0]?.hardness !== 'FIRM_RULE') {
    throw new Error(
      `finding-review-lifecycle.fullstack.spec: expected the seeded finding's rule_version to be FIRM_RULE, got ${hardness.rows[0]?.hardness ?? '(not found)'}`,
    );
  }
...
test.afterAll(async () => {
  ...
  // E2E-FULLSTACK-001 fixture). Comprehensive, dependency-ordered teardown
  // (established practice across PRs #303-#307) -- this spec's status change
  // (AC1) writes finding_status_event rows and AC3's reversal writes
  // human_override rows, both cleaned up precisely.
...
test('AC1+AC2: status change and a gating action both go through the real Dashboard, no full reload', async ({ page }) => {
  ...
  // The second half of AC2: an accepted OVERCHARGE finding is now eligible
  // for short-pay decisioning -- proven with the exact WHERE clause
  // generate-short-pay-decision.ts itself uses (src/modules/payments/generate-short-pay-decision.ts),
  // not a paraphrase of it.
  const shortPayEligible = await pool.query<{ id: string }>(
    `SELECT id FROM variance_finding
      WHERE client_id = $1 AND audit_run_id = $2 AND direction = 'OVERCHARGE' AND status = 'accepted'`,
    [DEV_CLIENT_ID, auditRunId],
  );
  expect(shortPayEligible.rows.map((r) => r.id)).toContain(findingId);
});

test('AC3: reversing a FIRM_RULE finding records a durable human_override row and never touches the finding\'s own status', async ({ request }) => {
  const beforeOverrideCount = await pool.query<{ n: number }>(
    `SELECT count(*)::int AS n FROM human_override WHERE client_id = $1 AND criterion_id = $2`,
    [DEV_CLIENT_ID, criterionId],
  );
  // Read whatever AC1+AC2 left this finding at (accepted) -- AC3 doesn't
  // care what the value IS, only that /reverse leaves it exactly as found,
  // proving the two mechanisms (action lifecycle vs. rule-governance
  // reversal) are genuinely separate, not the same thing under a different
  // name (the task's own reshape note).
  const statusBefore = await pool.query<{ status: string }>(`SELECT status FROM variance_finding WHERE id = $1`, [findingId]);
  ...
  const afterOverrideCount = await pool.query<{ n: number }>(
    `SELECT count(*)::int AS n FROM human_override WHERE client_id = $1 AND criterion_id = $2`,
    [DEV_CLIENT_ID, criterionId],
  );
  expect(afterOverrideCount.rows[0]!.n).toBe(beforeOverrideCount.rows[0]!.n + 1);
  ...
  // The load-bearing assertion this rebuild exists for: the finding's own
  // status is unaffected by the reversal write, whatever it was.
  const statusAfter = await pool.query<{ status: string }>(`SELECT status FROM variance_finding WHERE id = $1`, [findingId]);
  expect(statusAfter.rows[0]!.status).toBe(statusBefore.rows[0]!.status);
});
