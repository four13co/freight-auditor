...
describe('generateClaimEscalation (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const a = await owner.query(`INSERT INTO account (name, slug) VALUES ('GCE-A', $1) RETURNING id`, [`${tag}-a`]);
      clientAId = a.rows[0].id;
      const b = await owner.query(`INSERT INTO account (name, slug) VALUES ('GCE-B', $1) RETURNING id`, [`${tag}-b`]);
