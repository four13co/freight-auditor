...
export function ClientDisputeDetailView({ disputeId }: { disputeId: string | null }) {
  const { data: dispute, error } = useClientPortalResource<ClientPortalDisputeDetail>(
    () => (disputeId === null ? null : fetchClientPortalDispute(disputeId)),
    [disputeId],
  );

  const ready = disputeId !== null && (dispute !== null || error);
  ...
  return (
    <section data-testid="client-dispute-detail-view" className="border border-[rgba(32,30,29,.3)] bg-[#f3f2f2] p-3">
      ...
      <div aria-live="polite" aria-busy={disputeId !== null && !error && dispute === null} data-testid="client-dispute-detail-live-region">
        {disputeId === null && (
          <span role="status" data-testid="client-dispute-detail-empty" className="text-xs font-semibold text-[rgba(32,30,29,0.65)]">
            Select a dispute to view its detail.
          </span>
        )}

        {disputeId !== null && error && (
          <div ref={readyRef} tabIndex={-1} role="alert" data-testid="client-dispute-detail-error" className="text-xs font-semibold text-[#7c1405]">
            Couldn&rsquo;t load dispute.
          </div>
        )}

        {disputeId !== null && !error && dispute === null && (
          <span data-testid="client-dispute-detail-loading" className="text-xs font-semibold text-[rgba(32,30,29,0.65)]">Loading…</span>
        )}

        {disputeId !== null && !error && dispute !== null && (
          <div ref={readyRef} tabIndex={-1} data-testid="client-dispute-detail-content" className="text-xs">
            ...
            {dispute.lines.length === 0 && (
              <span role="status" data-testid="client-dispute-detail-lines-empty" className="text-xs font-semibold text-[rgba(32,30,29,0.65)]">
