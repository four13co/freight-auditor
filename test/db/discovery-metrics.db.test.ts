...
describe('discovery metrics (database)', () => {
  ...
  beforeAll(async () => {
    pool = makePool();
    clientId = (await pool.query(`INSERT INTO account(name,slug) VALUES('Discovery Metrics',$1) RETURNING id`, [tag])).rows[0].id;
    ...
    contractId = (await pool.query(`INSERT INTO contract(account_id,carrier_id,name) VALUES($1,$2,'Discovery Metrics') RETURNING id`,
    ...
    await pool.query(`INSERT INTO clarifying_question(account_id,source_document_id,question,field_path,
    ...
    await pool.query(`INSERT INTO extraction_field(account_id,source_document_id,field_path,ai_value,human_value)
  ...
  it('fails closed to zero rows under the app role when the tenant GUCs are never set (the PR #159 defect this guards against)', async () => {
    ...
    // admits a row only if app_is_internal() or the row's account_id is in
    // app_current_account_ids(). This transaction drops role WITHOUT calling
