...
describe('getRecoveryTraceability (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const c = await owner.query(`INSERT INTO account (name, slug) VALUES ('RT', $1) RETURNING id`, [tag]);
      ...
      const contract = await owner.query(`INSERT INTO contract (account_id, carrier_id, name) VALUES ($1, $2, 'RT contract') RETURNING id`, [clientId, carrierId]);
      const version = await owner.query(
        `INSERT INTO contract_version (account_id, contract_id, valid_from) VALUES ($1, $2, '2026-01-01') RETURNING id`,
        [clientId, contract.rows[0].id],
      );
      const clause = await owner.query(
        `INSERT INTO contract_clause (account_id, contract_version_id, clause_ref) VALUES ($1, $2, '4.2') RETURNING id`,
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await owner.query(`DELETE FROM contract_clause WHERE account_id = $1`, [clientId]);
  ...
  async function seedFinding(client: pg.PoolClient, opts: { withClause?: boolean }): Promise<string> {
    ...
    const vf = await client.query<{ id: string }>(
      `INSERT INTO variance_finding
         (account_id, audit_run_id, charge_fact_id, criterion_id, rule_version_id, direction, variance_amount, currency, status, evaluated_expr, clause_id)
  ...
  it('traces directly when the recovery_event carries a cited variance_finding_id', async () => {
    const result = await withTenantTx({ clientIds: [clientId], internal: true }, async (client) => {
      ...
      const claim = await client.query<{ id: string }>(`INSERT INTO claim (account_id, amount_claimed, currency, status) VALUES ($1, '100.0000', 'USD', 'open') RETURNING id`, [clientId]);
      const event = await client.query<{ id: string }>(
        `INSERT INTO recovery_event (account_id, claim_id, variance_finding_id, amount_recovered, currency) VALUES ($1, $2, $3, '100.0000', 'USD') RETURNING id`,
  ...
  it('traces indirectly via the claim dispute when no direct variance_finding_id is set', async () => {
    const result = await withTenantTx({ clientIds: [clientId], internal: true }, async (client) => {
      const findingId = await seedFinding(client, { withClause: true });
      const dispute = await client.query<{ id: string }>(`INSERT INTO dispute (account_id, carrier_id, status) VALUES ($1, $2, 'accepted') RETURNING id`, [clientId, carrierId]);
      await client.query(`INSERT INTO dispute_line (account_id, dispute_id, variance_finding_id, amount, currency) VALUES ($1, $2, $3, '100.0000', 'USD')`, [clientId, dispute.rows[0]!.id, findingId]);
      ...
      const event = await client.query<{ id: string }>(
        `INSERT INTO recovery_event (account_id, claim_id, amount_recovered, currency) VALUES ($1, $2, '100.0000', 'USD') RETURNING id`,
