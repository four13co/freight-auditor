...
describe('listAccountInvoices', () => {
  it('returns an empty array when the tenant has no invoices', async () => {
    ...
    const result = await listAccountInvoices(client, CLIENT_ID);
  ...
  it('threads a provided limit and offset through as query parameters', async () => {
    const { client, query } = mockClient([]);
    await listAccountInvoices(client, CLIENT_ID, { limit: 10, offset: 20 });
    const [, params] = query.mock.calls[0] as [string, unknown[]];
