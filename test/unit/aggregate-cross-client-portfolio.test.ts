...
describe('aggregateCrossClientPortfolio', () => {
  ...
  it('an open claim with no recovery is entirely outstanding', () => {
    ...
    expect(buckets[0]).toMatchObject({
      clientId: CLIENT_A, clientName: 'Client A', currency: 'USD',
      claimed: '500.0000', recovered: '0.0000', outstanding: '500.0000', writtenOff: '0.0000', denied: '0.0000',
    });
  ...
  it('groups by client separately -- the cross-client behavior this item adds', () => {
    ...
    expect(a.clientName).toBe('Client A');
    expect(b.claimed).toBe('200.0000');
    expect(b.clientName).toBe('Client B');
    assertReconciles(buckets);
  });
  ...
  it('aggregates multiple claims of mixed outcomes across multiple clients into the correct per-client buckets', () => {
    const buckets = aggregateCrossClientPortfolio(
      [
        ...
        claim({ claimId: 'c3', clientId: CLIENT_B, amountClaimed: '300.0000', status: 'denied' }),
        claim({ claimId: 'c4', clientId: CLIENT_B, amountClaimed: '400.0000', status: 'written_off' }),
      ],
      [
