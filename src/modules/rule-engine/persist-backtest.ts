...
export async function persistBacktest(client: pg.PoolClient, input: {
  clientId: string | null; ruleVersionId: string; result: BacktestResult;
}): Promise<{ id: string; created: boolean }> {
  const inserted = await client.query<{ id: string }>(`INSERT INTO rule_backtest
    (account_id, rule_version_id, corpus_hash, passed, pass_count, regression_count) VALUES ($1,$2,$3,$4,$5,$6)
    ON CONFLICT (account_id, rule_version_id, corpus_hash) DO NOTHING RETURNING id`,
  ...
  if (!id) {
    const row = (await client.query<{ id: string; passed: boolean; pass_count: number; regression_count: number }>(
      `SELECT id, passed, pass_count, regression_count FROM rule_backtest WHERE account_id=$1 AND rule_version_id=$2 AND corpus_hash=$3`,
  ...
  for (const item of input.result.cases) await client.query(`INSERT INTO rule_backtest_case
    (account_id, backtest_id, case_key, passed, input_hash, expected_hash, actual_hash, actual)
