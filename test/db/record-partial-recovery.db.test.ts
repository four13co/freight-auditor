...
describe('recordPartialRecovery -> reconciliation export enqueue (DB)', () => {
  ...
  /**
   * Round-67 QA finding on PR #284: the enqueue ran inline with no fault
   * isolation, so an enqueue failure rolled back the recovery_event write
   * with it -- a direct violation of the No-go "Don't let export enqueue
   * failure roll back or block the recovery_event write." This proves the
   * SAVEPOINT fix for real: a boss whose send() rejects must still let the
   * outer transaction commit and the recovery_event row persist.
   */
  it('still commits the recovery_event write when the export enqueue itself fails', async () => {
    const failingBoss: Pick<PgBoss, 'send'> = {
      send: vi.fn().mockRejectedValue(new Error('simulated pg-boss send failure')),
    } as unknown as Pick<PgBoss, 'send'>;

    const { claimId, recoveryEventId } = await withTenantTx({ clientIds: [clientId] }, async (client) => {
      const claimId = await seedClaim(client);
      const result = await recordPartialRecovery(client, failingBoss, {
        clientId,
        claimId,
        amountRecovered: '90.0000',
        currency: 'USD',
      });
      return { claimId, recoveryEventId: result.recoveryEventId };
    });
    ...
    expect(rows).toHaveLength(1);
    expect(rows[0].amount_recovered).toBe('90.0000');

    const expectedJobId = deterministicJobId(JOB_NAMES.EXPORT_RECORD_V1, clientId, `recovery-export:${recoveryEventId}`);
    const jobRows = await getPool().query(`SELECT id FROM pgboss.job WHERE name = $1 AND id = $2`, [JOB_NAMES.EXPORT_RECORD_V1, expectedJobId]);
