...
describe('getFindingsSummary (DB)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const a = await owner.query(`INSERT INTO account (name, slug) VALUES ('FS-A', $1) RETURNING id`, [`${tag}-a`]);
      clientAId = a.rows[0].id;
      const b = await owner.query(`INSERT INTO account (name, slug) VALUES ('FS-B', $1) RETURNING id`, [`${tag}-b`]);
  ...
  async function seedFinding(
    ...
    const vf = await client.query(
      `INSERT INTO variance_finding
         (account_id, audit_run_id, charge_fact_id, criterion_id, rule_version_id, direction, variance_amount, currency, status, evaluated_expr${createdAtClause})
       SELECT $1, $2, $3, c.id, rv.id, 'OVERCHARGE', $4, 'USD', $5, '{}'::jsonb${createdAtValue}
  ...
  it('AC1: recoveredLast30Days sums recovery_event.amount_recovered within the trailing 30 days only', async () => {
    const summary = await withTenantTx({ clientIds: [clientAId], internal: true }, async (c) => {
      ...
      await c.query(
        `INSERT INTO recovery_event (account_id, variance_finding_id, amount_recovered, currency, recorded_at)
