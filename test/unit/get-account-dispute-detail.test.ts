...
/**
 * Unit-level coverage of getAccountDisputeDetail's query-building, mirroring
 * get-dispute-detail.test.ts's structure exactly -- the added coverage here
 * is the explicit `account_id` predicate (86e31a9ch/#216 precedent) that
...
describe('getAccountDisputeDetail (unit, mocked client)', () => {
  it('returns null when the dispute does not exist / is not visible to this clientId', async () => {
    const client = mockClient({ disputeRows: [] });
    const result = await getAccountDisputeDetail(client, CLIENT_ID, DISPUTE_ID);
  ...
  it('binds disputeId and clientId as the dispute query params, in that order', async () => {
    ...
    await getAccountDisputeDetail(client, CLIENT_ID, DISPUTE_ID);
  ...
  it('binds disputeId and clientId as the dispute_line query params, in that order', async () => {
    ...
    expect(lineCall[0]).toMatch(/WHERE dispute_id = \$1 AND account_id = \$2/);
