...
/**
 * Fetches one claim plus its full recovery_event history (P5.B.4), scoped
 * to `clientId`. Returns null when the claim doesn't exist, isn't visible
 * under RLS, or belongs to a different client -- the caller maps that to
 * 404, matching update-finding-status.ts's `found: false` convention for
 ...
 * Same bug class #216 (86e31a9ch) fixed for extraction_field/
 * clarifying_question, which itself cites the original storeSourceDocument
 * cross-tenant fix as precedent.
 */
