...
function mockClient(opts: { claimRows?: unknown[]; docRows?: unknown[] }) {
  ...
  const query = vi.fn().mockImplementation((sql: string) => {
    if (sql.includes('FROM claim WHERE account_id')) return Promise.resolve({ rows: claimRows, rowCount: claimRows.length });
...
describe('listAccountClaimDocuments (unit, mocked client)', () => {
  it('returns null when the claim does not exist / is not visible to this clientId', async () => {
    const { client } = mockClient({ claimRows: [] });
    const result = await listAccountClaimDocuments(client, CLIENT_ID, CLAIM_ID);
  ...
  it('binds clientId and claimId as the claim-existence check params, in that order', async () => {
    ...
    await listAccountClaimDocuments(client, CLIENT_ID, CLAIM_ID);
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(sql).toMatch(/WHERE account_id = \$1 AND id = \$2/);
  ...
  it('binds clientId and claimId as the document-resolution query params, in that order', async () => {
    ...
    expect(docCall[0]).toMatch(/WHERE c\.account_id = \$1 AND c\.id = \$2/);
  ...
  it('joins through dispute_line and variance_finding so a findingless or document-less line is silently dropped, not an error', async () => {
    ...
    await listAccountClaimDocuments(client, CLIENT_ID, CLAIM_ID);
    const docCall = query.mock.calls.find((c) => (c[0] as string).includes('FROM claim c')) as [string, unknown[]];
    expect(docCall[0]).toMatch(/JOIN dispute_line dl ON dl\.account_id = c\.account_id AND dl\.dispute_id = c\.dispute_id/);
    expect(docCall[0]).toMatch(/JOIN variance_finding vf ON vf\.account_id = c\.account_id AND vf\.id = dl\.variance_finding_id/);
