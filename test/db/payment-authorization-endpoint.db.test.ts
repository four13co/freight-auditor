...
describe('POST /api/audit-runs/:id/payment-authorization (DB, e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const c = await owner.query(`INSERT INTO account (name, slug) VALUES ('PA', $1) RETURNING id`, [tag]);
  ...
  it('records an analyst approve decision and is idempotent on retry', async () => {
    ...
    try {
      const rows = await owner.query(
        `SELECT actor_kind FROM payment_gate_decision WHERE account_id = $1 AND audit_run_id = $2`,
        [clientId, auditRunId],
      );
