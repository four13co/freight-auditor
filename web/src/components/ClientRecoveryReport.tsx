...
export function ClientRecoveryReport() {
  ...
  return (
    <section data-testid="client-recovery-report" className="border border-[rgba(32,30,29,.3)] bg-[#f3f2f2] p-3">
      ...
      <div aria-live="polite" aria-busy={!error && buckets === null} data-testid="client-recovery-report-live-region">
        {error && (
          <span role="alert" data-testid="client-recovery-report-error" className="text-xs font-semibold text-[#7c1405]">
            Couldn&rsquo;t load recovery report.
          </span>
        )}

        {!error && buckets === null && (
          <span data-testid="client-recovery-report-loading" className="text-xs font-semibold text-[rgba(32,30,29,0.65)]">Loading…</span>
        )}

        {!error && buckets !== null && buckets.length === 0 && (
          <span role="status" data-testid="client-recovery-report-empty" className="text-xs font-semibold text-[rgba(32,30,29,0.65)]">
